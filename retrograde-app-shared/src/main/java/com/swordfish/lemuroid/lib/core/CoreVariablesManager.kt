...
class CoreVariablesManager(private val sharedPreferences: Lazy<SharedPreferences>) {
    suspend fun getOptionsForCore(
        ...
        // Respect GBA boot bios setting
        if (systemID == SystemID.GBA || systemCoreConfig.coreID == com.swordfish.lemuroid.lib.library.CoreID.MGBA) {
            val bootBiosEnabled = sharedPreferences.get().getBoolean("pref_key_boot_bios", true)
            if (bootBiosEnabled) {
                coreVariablesMap["mgba_use_bios"] = "ON"
                coreVariablesMap["mgba_skip_bios"] = "OFF"
            } else {
                coreVariablesMap["mgba_use_bios"] = "OFF"
                coreVariablesMap["mgba_skip_bios"] = "ON"
            }
        }
