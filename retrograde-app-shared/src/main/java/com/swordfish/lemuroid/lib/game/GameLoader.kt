    ...
    fun load(
        ...
        flow {
            try {
                ...
                val sharedPrefs = com.swordfish.lemuroid.lib.preferences.SharedPreferencesHelper.getSharedPreferences(appContext)
                val bootBiosEnabled = sharedPrefs.getBoolean("pref_key_boot_bios", true)
                ...
                val quickSaveData =
                    runCatching {
                        ...
                        if (systemCoreConfig.statesSupported && loadSave && shouldDiscardSave && !bootBiosEnabled) {
                ...
                // Auto copy gba_bios.bin from assets or alternate named files if present
                val biosFile = File(systemDirectory, "gba_bios.bin")
                if (!biosFile.exists() || biosFile.length() == 0L) {
                    systemDirectory.mkdirs()
                    val potentialBios = listOf("gbabios.bin", "gba.bios", "bios.bin", "bios_gba.bin", "normatt_bios.bin", "GBA_BIOS.BIN")
                    val existingAlt = potentialBios.map { File(systemDirectory, it) }.firstOrNull { it.exists() && it.length() > 0 }
                    if (existingAlt != null) {
                        runCatching {
                            existingAlt.copyTo(biosFile, overwrite = true)
                        }
                    } else {
                        ...
                        for (candidate in assetCandidates) {
                            val copied = runCatching {
                                appContext.assets.open(candidate).use { input ->
                                    biosFile.outputStream().use { output ->
                                        input.copyTo(output)
                                    }
                                }
                                biosFile.exists() && biosFile.length() > 0L
                            }.getOrDefault(false)
                            if (copied) {
                                Timber.i("Copied GBA BIOS from asset: $candidate")
                                break
                            }
                        }
                    }
