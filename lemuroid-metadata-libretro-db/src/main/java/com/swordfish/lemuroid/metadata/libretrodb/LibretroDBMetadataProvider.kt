...
class LibretroDBMetadataProvider(private val ovgdbManager: LibretroDBManager) :
    ...
    override suspend fun retrieveMetadata(storageFile: StorageFile): GameMetadata? {
        ...
        val ext = storageFile.extension.lowercase(Locale.US)
        // Strict GBA-only filter: ignore all non-GBA files completely
        if (ext != "gba" && ext != "agb") {
            Timber.d("Ignoring non-GBA file extension: $ext for file: ${storageFile.name}")
            return null
        }

        val gbaSystem = GameSystem.findByUniqueFileExtension(ext) ?: return null

        // 1. Database lookup (CRC, Serial, Filename, etc.) to enrich name/boxart if available
        val dbMatch = findInDatabase(storageFile)
        if (dbMatch != null && dbMatch.system == "gba") {
            Timber.d("GBA Metadata retrieved from DB: $dbMatch")
            return dbMatch
        }

        // 2. Direct GBA metadata fallback
        val fallbackMetadata = GameMetadata(
            name = storageFile.extensionlessName,
            romName = storageFile.name,
            thumbnail = computeCoverUrl(gbaSystem, storageFile.extensionlessName),
            system = "gba",
            developer = null,
        )
        Timber.d("GBA Metadata retrieved: $fallbackMetadata")
        return fallbackMetadata
    }

    private suspend fun findInDatabase(storageFile: StorageFile): GameMetadata? {
        val db = runCatching { ovgdbManager.dbInstance }.getOrNull() ?: return null
        return runCatching {
